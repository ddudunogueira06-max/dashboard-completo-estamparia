...
export const WIDGETS: WidgetDef[] = [
  ...
  /* ---------------------- Dobra ---------------------- */
  {
    ...
    Component: () => {
      ...
      const ultimo = data.reduce<string>((mx, c) => (c.data_conclusao && c.data_conclusao > mx ? c.data_conclusao : mx), "");
...
export function SeriesChart({
  ...
  const padrao = temporal && def.data.length > 30 ? def.data.slice(-30) : def.data;
  const data: Record<string, unknown>[] = porData ?? (dias > 0 && temporal ? def.data.slice(-dias) : padrao);
