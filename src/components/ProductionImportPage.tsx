...
interface ImportRecord {
  id: string; filename: string; total_rows: number; inserted_rows: number;
  skipped_rows: number; status: string; error_message: string | null; created_at: string;
}

async function fetchImports(): Promise<ImportRecord[]> {
  const { data, error } = await supabase
    .from("production_imports").select("*")
    .order("created_at", { ascending: false }).limit(50);
  if (error) throw error;
  return (data ?? []) as ImportRecord[];
}

export function ProductionImportPage() {
  ...
  const { data: imports = [] } = useQuery({ queryKey: ["production_imports"], queryFn: fetchImports });

  async function handleImport() {
    if (!file) return;
    setBusy(true); setFeedback(null); setProgress("Lendo planilha…");
    try {
      const rows = await parseProductionFile(file);
      if (rows.length === 0) throw new Error("Nenhuma linha válida encontrada (aba BASE).");

      if (mode === "replace") {
        setProgress("Removendo dados existentes…");
        const { error: delErr } = await supabase.from("production_records").delete().neq("id", "00000000-0000-0000-0000-000000000000");
      ...
      await supabase.from("production_imports")
        .update({ inserted_rows: inserted, skipped_rows: skipped, status: "success" })
        .eq("id", importId);

      setFeedback({ type: "success", msg: `Importação concluída! ${fmtInt(inserted)} registros novos${skipped > 0 ? `, ${fmtInt(skipped)} ignorados` : ""}.` });
      setFile(null);
  ...
  async function handleDeleteAll() {
    ...
    setBusy(true);
    const { error } = await supabase.from("production_records").delete().neq("id", "00000000-0000-0000-0000-000000000000");
    setBusy(false);
    if (error) setFeedback({ type: "error", msg: error.message });
    else { setFeedback({ type: "success", msg: "Todos os registros foram removidos." }); qc.invalidateQueries({ queryKey: ["production_records"] }); }
  }

  return (
    <div className="p-6 space-y-6 max-w-6xl">
      <header>
        <h1 className="text-2xl font-bold tracking-tight">Importar Planilha de Produção</h1>
        <p className="text-sm text-muted-foreground">Envie a planilha (.xlsx) com a aba <code className="text-primary">BASE</code> para alimentar o painel de capacidade.</p>
      </header>

      <section className="bg-card border border-border rounded-xl p-6">
        <div
          onDragOver={(e) => e.preventDefault()}
          onDrop={async (e) => { e.preventDefault(); const f = e.dataTransfer.files?.[0]; if (f) { setFile(f); setPreviewCount((await parseProductionFile(f)).length); } }}
        ...
        >
          <input ref={fileRef} type="file" accept=".xlsx,.xlsm,.xls" className="hidden"
            onChange={async (e) => { const f=e.target.files?.[0] ?? null; setFile(f); setPreviewCount(f ? (await parseProductionFile(f)).length : null); }} />
          {file ? (
              ...
              {imports.map((i) => (
                <tr key={i.id} className="border-t border-border hover:bg-secondary/30">
                  ...
                  <td className="px-3 py-2">
                    <span className={`text-xs ${i.status === "success" ? "text-success" : i.status === "error" ? "text-destructive" : "text-warning"}`}>{i.status}</span>
                  </td>
                </tr>
              ))}
              {imports.length === 0 && (
